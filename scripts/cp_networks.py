...
from scripts import cp_network_lora
...
module_types = [
    cp_network_lora.ModuleTypeLora(),
]
...
def load_network(name, network_on_disk):
    ...
    for key_network, weight in sd.items():
        ...
        if key not in matched_networks:
            matched_networks[key] = cp_network.NetworkWeights(network_key=key_network, sd_key=key, w={}, sd_module=sd_module)
    ...
    for key, weights in matched_networks.items():
        ...
        for nettype in module_types:
            net_module = nettype.create_module(net, weights) # invoke cp_network_lora.py: 12
...
last_req_params = [shared.opts.data["sd_model_checkpoint"], [], [], [], []]
lora_status = 0 # no lora is 0, lora first run is 1, lora second/third run is -1

def need_reblade(names, te_multipliers=None, unet_multipliers=None, dyn_dims=None):
    global last_req_params
    backbone_ckpt_name, lr_names, lr_te_multipliers, lr_unet_multipliers, lr_dyn_dims = last_req_params
    current_ckpt_name = shared.opts.data["sd_model_checkpoint"]
    if backbone_ckpt_name != current_ckpt_name:
        last_req_params = [current_ckpt_name, names, te_multipliers, unet_multipliers, dyn_dims]
        return True
    if lr_names != names or lr_te_multipliers != te_multipliers or \
            lr_unet_multipliers != unet_multipliers or lr_dyn_dims != dyn_dims:
        last_req_params = [current_ckpt_name, names, te_multipliers, unet_multipliers, dyn_dims]
        return True
    return False
...
def network_apply_weights(self: Union[torch.nn.Conv2d, torch.nn.Linear, torch.nn.MultiheadAttention]):
    ...
    if current_names != wanted_names:
        ...
        self.cp_network_current_names = wanted_names
...
def list_available_networks():
    ...
    candidates += list_lora_models()
    if shared.cmd_opts.uid is not None:
        uid_lora = os.path.join(shared.cmd_opts.data_dir, 'models/Lora')
        if uid_lora!=shared.cmd_opts.lora_dir:
            os.makedirs(uid_lora, exist_ok=True)
            candidates += list(shared.walk_files(uid_lora, allowed_extensions=[".pt", ".ckpt", ".safetensors"]))
    names = set()
    for filename in candidates:
        ...
        if name in names: 
            ...
            names.add(name)
        try:
            entry = cp_network.NetworkOnDisk(name, filename)
